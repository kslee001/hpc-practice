...
/* KERNELS */
// Fused Kernel
__global__ void ConvBNReLUFusedKernel(float *input, float *weight, float *bias,
...
void uNet(Tensor *inputN, Tensor *outputN, int N) {
  ...
  for (int batch = 0; batch < num_batches; ++batch) {
    ...
    int batch_end = min((batch + 1) * BATCH_SIZE, N);
    int num_items = batch_end - batch_start;

    // define streams and events for this batch
    ...
    // copy input to device
      CHECK_CUDA(cudaMemcpyAsync(
        input->buf_gpu,
        inputN->buf + (batch_start *3*128*191),
        sizeof(float) * num_items *3*128*191,
        cudaMemcpyHostToDevice,
        data_stream
      ));

      // event record for data stream
      cudaEventRecord(input_end, data_stream);
      cudaStreamWaitEvent(comp_stream, input_end);
      ...
      Conv2dDevice(inc_batchnorm_0_output, inc_double_conv_3_weight, NULL,
            inc_conv_1_output, 1, 1, 1, false, comp_stream);
      BatchNorm2dDevice(inc_conv_1_output, inc_double_conv_4_weight,
                  inc_double_conv_4_bias, inc_batchnorm_1_running_mean,
                  inc_batchnorm_1_running_var, inc_batchnorm_1_output, 1e-5, 0.1, comp_stream);
      ReLUDevice(inc_batchnorm_1_output, comp_stream);
      ...
      cudaMemcpyAsync(
        outputN->buf + (batch_start  * 2 * 128 * 191),
        output->buf_gpu,
        sizeof(float) * num_items  * 2 * 128 * 191,
