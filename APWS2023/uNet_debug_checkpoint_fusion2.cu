...
// blockdim = output's H*W
// block idx = batch * filters


/* KERNELS */
__global__ void ConvBNReLUFusedKernel(
  float *input, float *weight, float *bias,
  float *gamma, float *beta, float *running_mean,
  float *running_var, float *output,
  ...
  float sum = bias != nullptr ? bias[oc] : 0;
  ...
  float variance = running_var[oc];
  float x_hat = (sum - mean) / sqrt(variance + eps);
  float bn_output = gamma[oc] * x_hat + beta[oc];

  output[((oc * OH + oh) * OW) + ow] = bn_output > 0 ? bn_output : 0;
}

void ConvBNReLUDevice(Tensor *input, Tensor *weight, Tensor *bias,
                      Tensor *gamma, Tensor *beta, Tensor *running_mean,
                      Tensor *running_var, Tensor *output,
                      int stride, int pad, int dilation,
                      const float eps, const float momentum,
                      cudaStream_t s) {
  ...
  ConvBNReLUFusedKernel<<<gridDim, blockDim, 0, s>>>(
      input->buf_gpu, weight->buf_gpu, bias != nullptr ? bias->buf_gpu : nullptr,
      gamma->buf_gpu, beta->buf_gpu, running_mean->buf_gpu, running_var->buf_gpu, output->buf_gpu,
      N, C, H, W, K, R, S, ON, OC, OH, OW, stride, pad, dilation, eps, momentum
  );
}
...
__global__ void Conv2dKernel_NC(
  ...
  for(int oh=0; oh<OH; ++oh){
    for(int ow = 0; ow < OW; ++ow){
      ...
      output[((on*OC+oc)*OH+oh)*OW+ow] = sum;
      }
    }
  }
}

void Conv2dDevice_NC(Tensor *input, Tensor *weight, Tensor *bias, Tensor *output,
                             int stride, int pad, int dilation, bool has_bias, cudaStream_t s){
  int N=input->shape[0], C=input->shape[1], H=input->shape[2], W=input->shape[3];
  int K=weight->shape[0], R=weight->shape[2], S=weight->shape[3];
  const int ON = N;
  const int OC = K;
  ...
  dim3 blockDim(BLOCK_SIZE, BLOCK_SIZE);
  dim3 gridDim((input->shape[0]+BLOCK_SIZE-1)/BLOCK_SIZE, (weight->shape[0]+BLOCK_SIZE-1)/BLOCK_SIZE);

  Conv2dKernel_NC<<<gridDim, blockDim, 0, s>>>(
...
void Conv2dDevice(Tensor *input, Tensor *weight, Tensor *bias, Tensor *output,
  ...
  Conv2dKernel<<<gridDim, blockDim, 0, s>>>(
    input->buf_gpu, weight->buf_gpu, b, output->buf_gpu,
    N, C, H, W, K, R, S, ON, OC, OH, OW, stride, pad, dilation, has_bias
  );
}
...
void BatchNorm2dDevice(Tensor *input, Tensor *gamma, Tensor *beta,
  ...
  BatchNorm2dKernel<<<gridDim, blockDim, 0, s>>>(
...
__global__ void BatchNorm2dKernel_SUCK(float *input, float *gamma, float *beta,
                                  float *running_mean, float *running_var, float *output,
                                  int N, int C, int H, int W,
                                  const float eps, const float momentum) {
  for (int c = 0; c < C; ++c) {
...
void BatchNorm2dDevice_SUCK(Tensor *input, Tensor *gamma, Tensor *beta,
                       Tensor *running_mean, Tensor *running_var, Tensor *output,
                       const float eps, const float momentum, cudaStream_t s) {
...
void MaxPool2dDevice(Tensor *input, Tensor *output, cudaStream_t s) {
  int C = input->shape[1], H = input->shape[2], W = input->shape[3];
  int OH = output->shape[2], OW = output->shape[3];

  dim3 blockDim(OW);
  dim3 gridDim(C, OH);
  MaxPool2dKernel<<<gridDim, blockDim, 0, s>>>(input->buf_gpu, output->buf_gpu, C, H, W, OH, OW);
...
void ConvTranspose2dDevice(Tensor *input, Tensor *weight, Tensor *bias,
                           Tensor *output, int stride, int pad, cudaStream_t s) {
    int N = input->shape[0], C = input->shape[1], H = input->shape[2], W = input->shape[3];
    int K = weight->shape[1], R = weight->shape[2], S = weight->shape[3];
    int OH = output->shape[2], OW = output->shape[3];

    CHECK_ERROR(OH == (H - 1) * stride - 2 * pad + R, "[ConvT2d] Output height mismatch");
    CHECK_ERROR(OW == (W - 1) * stride - 2 * pad + S, "[ConvT2d] Output width mismatch");
    CHECK_ERROR(weight->shape[0] == C && bias->shape[0] == K && output->shape[1] == K,
                "[ConvT2d] Channel size mismatch");

    dim3 blockDim(BLOCK_SIZE, BLOCK_SIZE);
    dim3 gridDim((OH + blockDim.x - 1) / blockDim.x, (OW + blockDim.y - 1) / blockDim.y, K);
    ConvTranspose2dKernel<<<gridDim, blockDim, 0, s>>>(
...
__global__ void ConvTranspose2dKernel_SUCK(
    float *input, float *weight, float *bias, float *output,
    int N, int C, int H, int W, int K, int R, int S, int OH, int OW,
    int stride, int pad
) {
    ...
    for (int oh = 0; oh < OH; ++oh) {
        for (int ow = 0; ow < OW; ++ow) {
            ...
            output[(n * K + k) * OH * OW + oh * OW + ow] = sum;
        }
    }
}

void ConvTranspose2dDevice_SUCK(Tensor *input, Tensor *weight, Tensor *bias,
                           Tensor *output, int stride, int pad, cudaStream_t s) {
    int N = input->shape[0], C = input->shape[1], H = input->shape[2], W = input->shape[3];
    int K = weight->shape[1], R = weight->shape[2], S = weight->shape[3];
    int OH = output->shape[2], OW = output->shape[3];
...
void ConcatDevice(Tensor *input1, Tensor *input2, Tensor *output, cudaStream_t s) {
  ...
  ConcatKernel<<<gridDim, blockDim, 0, s>>>(
...
__global__ void ConcatKernel_SUCK(
    float *input1, float *input2, float *output,
    int C1, int H1, int W1, int C2, int H2, int W2,
    int OC, int OH, int OW) {
...
void ConcatDevice_SUCK(Tensor *input1, Tensor *input2, Tensor *output, cudaStream_t s) {
  int C1 = input1->shape[1], H1 = input1->shape[2], W1 = input1->shape[3];
  int C2 = input2->shape[1], H2 = input2->shape[2], W2 = input2->shape[3];
  int OC = output->shape[1], OH = output->shape[2], OW = output->shape[3];
...
void uNet(Tensor *inputN, Tensor *outputN, int N) {
  ...
  Tensor *output = new Tensor({BATCH_SIZE, 2, 128, 191});
    ...
    cudaStream_t data_stream  = streams[1];
    cudaStream_t comp_stream = streams[0];

    cudaEvent_t input_end = events[0];
    cudaEvent_t comp_end  = events[1];

    cudaMemcpyAsync(input->buf_gpu,
    ...
    memcpy(input->buf,
      inputN->buf+(idx*1*3*128*191),
      sizeof(float)*BATCH_SIZE*3*128*191);
    debug(input, "input");


    // evene record for data stream
    cudaEventRecord(input_end, data_stream);  // data_stream 끝나면 input_end 켜라
    cudaStreamWaitEvent(comp_stream, input_end);    // comp_stream을 시작해라, input_end가 켜졌으면
    ...
    //        false, comp_stream);
    // debug(inc_batchnorm_1_output, "INC conv2d 1");
    ...
    //             inc_double_conv_1_bias, inc_batchnorm_0_running_mean,
    //             inc_batchnorm_0_running_var, inc_batchnorm_0_output, 1e-5, 0.1, comp_stream);
    // debug(inc_batchnorm_1_output, "INC batchnorm 1");

    ReLU(inc_batchnorm_0_output);
    // ReLUDevice(inc_batchnorm_0_output, comp_stream);
    ConvBNReLUDevice(
      // Tensor *input, Tensor *weight, Tensor *bias,
      input, inc_double_conv_0_weight, NULL,
      // Tensor *gamma, Tensor *beta, Tensor *running_mean,
      inc_double_conv_1_weight, inc_double_conv_1_bias, inc_batchnorm_0_running_mean,
      // Tensor *running_var, Tensor *output,
      inc_batchnorm_0_running_var, inc_batchnorm_0_output,
      // int stride, int pad, int dilation,
      1, 1, 1,
      // const float eps, const float momentum,
      1e-5, 0.1,
    ...
    debug(inc_batchnorm_1_output, "INC convrelu1");
    ...
    debug(inc_batchnorm_1_output, "INC convrelu2");

    // down1(64, 128)
    MaxPool2d(inc_batchnorm_1_output, down1_maxpool2d_0_output);
    MaxPool2dDevice(inc_batchnorm_1_output, down1_maxpool2d_0_output, comp_stream);
    ...
    Conv2dDevice(down1_maxpool2d_0_output, down1_maxpool_conv_1_double_conv_0_weight,
              NULL, down1_conv_0_output, 1, 1, 1, false, comp_stream);
    ...
    BatchNorm2dDevice(down1_conv_0_output, down1_maxpool_conv_1_double_conv_1_weight,
                down1_maxpool_conv_1_double_conv_1_bias,
                down1_batchnorm_0_running_mean, down1_batchnorm_0_running_var,
                down1_batchnorm_0_output, 1e-5, 0.1, comp_stream);

    ReLU(down1_batchnorm_0_output);
    ReLUDevice(down1_batchnorm_0_output, comp_stream);
    ...
    Conv2dDevice(down1_batchnorm_0_output, down1_maxpool_conv_1_double_conv_3_weight,
           NULL, down1_conv_1_output, 1, 1, 1, false, comp_stream);
    ...
    BatchNorm2dDevice(down1_conv_1_output, down1_maxpool_conv_1_double_conv_4_weight,
                down1_maxpool_conv_1_double_conv_4_bias,
                down1_batchnorm_1_running_mean, down1_batchnorm_1_running_var,
                down1_batchnorm_1_output, 1e-5, 0.1, comp_stream);

    ReLU(down1_batchnorm_1_output);
    ReLUDevice(down1_batchnorm_1_output, comp_stream);
    debug(down1_batchnorm_1_output, "down1 relu");


    // down2(128, 256)
    MaxPool2d(down1_batchnorm_1_output, down2_maxpool2d_0_output);
    MaxPool2dDevice(down1_batchnorm_1_output, down2_maxpool2d_0_output, comp_stream);
    ...
    Conv2dDevice(down2_maxpool2d_0_output, down2_maxpool_conv_1_double_conv_0_weight,
           NULL, down2_conv_0_output, 1, 1, 1, false, comp_stream);
    ...
    BatchNorm2dDevice(down2_conv_0_output, down2_maxpool_conv_1_double_conv_1_weight,
                down2_maxpool_conv_1_double_conv_1_bias,
                down2_batchnorm_0_running_mean, down2_batchnorm_0_running_var,
                down2_batchnorm_0_output, 1e-5, 0.1, comp_stream);

    ReLU(down2_batchnorm_0_output);
    ReLUDevice(down2_batchnorm_0_output, comp_stream);
    ...
    Conv2dDevice(down2_batchnorm_0_output, down2_maxpool_conv_1_double_conv_3_weight,
           NULL, down2_conv_1_output, 1, 1, 1, false, comp_stream);
    ...
    BatchNorm2dDevice(down2_conv_1_output, down2_maxpool_conv_1_double_conv_4_weight,
                down2_maxpool_conv_1_double_conv_4_bias,
                down2_batchnorm_1_running_mean, down2_batchnorm_1_running_var,
                down2_batchnorm_1_output, 1e-5, 0.1, comp_stream);


    ReLU(down2_batchnorm_1_output);
    ReLUDevice(down2_batchnorm_1_output, comp_stream);
    ...
    ConvTranspose2dDevice(down2_batchnorm_1_output, up1_up_weight, up1_up_bias,
                    up1_convt_0_output, 2, 0, comp_stream);

    Concat(up1_convt_0_output, down1_batchnorm_1_output, up1_concat_0_output);
    ConcatDevice(up1_convt_0_output, down1_batchnorm_1_output, up1_concat_0_output, comp_stream);
    ...
    Conv2dDevice(up1_concat_0_output, up1_conv_double_conv_0_weight, NULL,
           up1_conv_0_output, 1, 1, 1, false, comp_stream);
    ...
    BatchNorm2dDevice(up1_conv_0_output, up1_conv_double_conv_1_weight,
                up1_conv_double_conv_1_bias, up1_batchnorm_0_running_mean,
                up1_batchnorm_0_running_var, up1_batchnorm_0_output, 1e-5, 0.1, comp_stream);
    debug(up1_batchnorm_0_output, "up1_batchnorm_0_output");

    ReLU(up1_batchnorm_0_output);
    ReLUDevice(up1_batchnorm_0_output, comp_stream);
    ...
    Conv2dDevice(up1_batchnorm_0_output, up1_conv_double_conv_3_weight, NULL,
           up1_conv_1_output, 1, 1, 1, false, comp_stream);
    ...
    BatchNorm2dDevice(up1_conv_1_output, up1_conv_double_conv_4_weight,
                up1_conv_double_conv_4_bias, up1_batchnorm_1_running_mean,
                up1_batchnorm_1_running_var, up1_batchnorm_1_output, 1e-5, 0.1, comp_stream);

    ReLU(up1_batchnorm_1_output);
    ReLUDevice(up1_batchnorm_1_output, comp_stream);
    ...
    ConvTranspose2dDevice(up1_batchnorm_1_output, up2_up_weight, up2_up_bias,
                    up2_convt_0_output, 2, 0, comp_stream);


    Concat(up2_convt_0_output, inc_batchnorm_1_output, up2_concat_0_output);
    ConcatDevice(up2_convt_0_output, inc_batchnorm_1_output, up2_concat_0_output, comp_stream);
    ...
    Conv2dDevice(up2_concat_0_output, up2_conv_double_conv_0_weight, NULL,
           up2_conv_0_output, 1, 1, 1, false, comp_stream);
    ...
    BatchNorm2dDevice(up2_conv_0_output, up2_conv_double_conv_1_weight,
                up2_conv_double_conv_1_bias, up2_batchnorm_0_running_mean,
                up2_batchnorm_0_running_var, up2_batchnorm_0_output, 1e-5, 0.1, comp_stream);
    ReLU(up2_batchnorm_0_output);
    ReLUDevice(up2_batchnorm_0_output, comp_stream);
    ...
    Conv2dDevice(up2_batchnorm_0_output, up2_conv_double_conv_3_weight, NULL,
           up2_conv_1_output, 1, 1, 1, false, comp_stream);
    ...
    BatchNorm2dDevice(up2_conv_1_output, up2_conv_double_conv_4_weight,
                up2_conv_double_conv_4_bias, up2_batchnorm_1_running_mean,
                up2_batchnorm_1_running_var, up2_batchnorm_1_output, 1e-5, 0.1, comp_stream);
    ReLU(up2_batchnorm_1_output);
    ReLUDevice(up2_batchnorm_1_output, comp_stream);
    ...
    Conv2dDevice(up2_batchnorm_1_output, outc_conv_weight, outc_conv_bias, output, 1,
           0, 1, true, comp_stream);

    cudaEventRecord(comp_end, comp_stream);

    debug(output, "output");

    cudaStreamWaitEvent(data_stream, comp_end);
    cudaMemcpyAsync(
