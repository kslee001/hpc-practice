...
// void matvecKernel(
//   float* input1, float* input2, float* output,
//   const int N_, const int K_, const int M_
//   ){
...
//     acc[i] += input1[cur_row*N_ + i]*input2[i*K_ + cur_col];
//   }
//   __syncthreads();
...
//   const int N = output->shape[0]; // batch size

//   dim3 blockDim(BLOCK_SIZE, BS); // x, y
...
void matmulKernel(
  ...
  __shared__ float SUB1[BLOCK_SIZE][BLOCK_SIZE];
  __shared__ float SUB2[BLOCK_SIZE][BLOCK_SIZE];

  const int ly = threadIdx.y;
  const int lx = threadIdx.x; // local x
  const int gy = blockIdx.y * BLOCK_SIZE + ly;
  const int gx = blockIdx.x * BLOCK_SIZE + lx; // global x
  ...
  int numThreads = (M+BLOCK_SIZE-1)/BLOCK_SIZE; //ceiling
  for(int thread=0; thread<numThreads; ++thread){
    for(int w=0; w<WPT; ++w){
      const int ty = thread*BLOCK_SIZE + ly;
      const int tx = thread*BLOCK_SIZE + lx;

      if ((gy + w*RTS) < M && tx < K){
        SUB1[ly + w*RTS][lx] = input1[(gy + w*RTS)*M + tx];
      }
      else{
        SUB1[ly + w*RTS][lx] = 0.0f;
      }

      if((ty + w * RTS) < N && gx < K){
        SUB2[ly + w*RTS][lx] = input2[(ty + w*RTS)*K + gx];
      }
      else{
        SUB2[ly + w*RTS][lx] = 0.0f;
      }
    }
    __syncthreads();
    for (int k=0; k<BLOCK_SIZE; ++k){
      for(int w=0; w<WPT; ++w){
        acc[w] += SUB1[ly + w*RTS][k]*SUB2[k][lx];
      }
    }
    __syncthreads();
  }
...
void matmulDevice(Tensor *input1, Tensor *input2, Tensor *output){
  ...
  matmulKernel<<<gridDim, blockDim>>>(
    input1->buf, input2->buf, output->buf, M, K, N
  );
  cudaDeviceSynchronize();
}
